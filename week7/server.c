...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* restAfterQuerySign = strchr(rest, '?');
    char* abs_path_stop = strchr(rest, ' ');
    // printf("abs_path_stop0 =%s \n", abs_path_stop);

    if (restAfterQuerySign != NULL) {
        abs_path_stop = restAfterQuerySign;
        char *nextSpace = strchr(restAfterQuerySign, ' ');
        query = strncpy(query, restAfterQuerySign + 1, nextSpace - restAfterQuerySign);
        const char *needle = "\"";
    ...
    if (!strcmp(++http_version_start, "HTTP/1.1") == 0) {
